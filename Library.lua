...
-- library init
	...
	writefile("ffff.ttf", game:HttpGet("https://github.com/imnothereanymore2/secrethaxx-atlanta-ui-real-fake/blob/main/tahoma.ttf"))
		...
		function library:window(properties)
			...
			-- main window
				local main_window = library:panel({
					name = properties and properties.name or "secrethaxx | ", 
			...
			-- theming 
				...
				local watermark = library:watermark({default = os.date('secrethaxx |  - %b %d %Y - %H:%M:%S')})  

				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date('secrethaxx - Beta - %b %d %Y - %H:%M:%S'))
					end 
				end) 
